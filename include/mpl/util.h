...
/*
    Faster way to do this is to sort the types by typeid and then determine
   uniqueness in O(n) but this is rarely used for large types like that...
*/
template <typename Head, typename... Rest> struct is_unique<Head, Rest...> {
  static constexpr bool value =
      !contains<Head, Rest...>::value && is_unique<Rest...>::value;
};
...
template <long N, typename Target, typename... Ts> struct index_of_impl;
...
template <long N, typename Target> struct index_of_impl<N, Target> {
...
struct find_if<Predicate, Head, Rest...> {
  using type = std::conditional_t<Predicate<Head>::value, Head,
                                  typename find_if<Predicate, Rest...>::type>;
};
template <template <typename> typename Predicate> struct find_if<Predicate> {
  using type = types::type_not_found;
};
